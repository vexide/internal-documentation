...
VEX has released both a public version of the V5's SDK (`libv5rt`) and a private version that's exclusively available to VEX partners (`libv5rts`). Developers who have been provided access to the Partner SDK and its documentation have historically been placed under an NDA, but we can infer from analysis of compiled PROS framework binaries that it consists of a stripped-down version of the public SDK with the VEXcode framework removed.
